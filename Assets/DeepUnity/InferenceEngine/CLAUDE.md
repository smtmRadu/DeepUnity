# DeepUnity InferenceEngine — full-GPU model inference inside Unity

The subsystem of DeepUnity that runs PRETRAINED neural models (LLM / TTS / STT) entirely on the
GPU through dedicated HLSL compute kernels — **no ONNX Runtime, no Sentis, no native plugins, no
Python at runtime**. A shipped game contains only C# + .compute shaders + exported `.bin` weights.
This file is the source of truth for the architecture, conventions, and progress. Update it as
milestones land.

## Structure (F3 move landed 2026-07-12)

```
Assets/DeepUnity/InferenceEngine/           ← F3 restructure landed 2026-07-12
  CLAUDE.md                ← this file
  import_params.py         ← THE unified exporter + model pool registry (`--list`); LLM+TTS+STT
                             (pocket-tts exporter merged in 2026-07-13; standalone
                             TTS/PocketTTS/validation/import_pocket_tts.py removed — kokoro/STT
                             exporters still pending merge from their workstreams)
  LLM/
    Base/  (LLM.cs, LLMConfig.cs, BPETokenizer.cs)  Qwen3_5/  Gemma3/  MiniCPM5/
  TTS/
    TTS.cs (family base)   Chatterbox/  CosyVoice/  Kokoro/  VoiceLab/  validation per module
  STT/
    STT.cs (family base)   QwenASR/  Parakeet/
Assets/DeepUnity/Main/      ModelBase.cs, TokenizerBase.cs (family roots — shared beyond InferenceEngine)
Assets/Resources/ComputeShaders/  one .compute per model family, ALL registered in Main/DeepUnityMeta.cs
Assets/Resources/Weights/weights_<model>_<size>_<quant>/   exported manifests, FLAT (gitignored)
```

Note (F3, 2026-07-12): the three families were moved from `Assets/DeepUnity/{LLM,TTS,STT}/`
into `Assets/DeepUnity/InferenceEngine/`. Namespaces stayed `DeepUnity` (folder ≠ namespace in
C#), so no code references changed — but ~77 hardcoded asset-path STRINGS (tokenizer JSONs,
validation dump dirs, KokoroG2P lexicon dir, VoiceLab presets, legacy weight fallbacks) were
rewritten to the new roots. `ModelBase` / `TokenizerBase` stay under `Main/` as engine-wide roots.

## Architecture (the rules every module follows)

1. **Class hierarchy** — `ModelBase` → abstract family (`LLM`/`TTS`/`STT`) → concrete model.
   Tokenizers: `TokenizerBase` → `TextTokenizer`/`SpeechTokenizer` → concrete (WS-F2).
2. **GPU residency lifecycle on ModelBase** ("latent loading"; dissertation feature):
   `Unloaded ⇄ Prefetching → Ready`, all transitions anti-frame-drop.
   `Prefetch() / SlowPrefetch(sec) / BoostFetch() / PausePrefetch() / Defetch(Full|Slow) /
   DefaultDefetchMode / LoadProgress / Residency`. Loader-side: live per-instance byte budget
   sampled every frame; epoch-invalidated IO so mid-load Defetch is race-free; prefetch during
   defetch is queued, never lost. Reference impl: `TTS/CosyVoice/CosyVoiceWeights.cs`.
3. **Weights**: exported by `import_params.py` (pool registry: `python import_params.py
   --list`) into per-tensor `.bin` + `manifest.tsv` (`name\tfile\tdtype\tnumel\tshape`).
   fp16 packed 2-per-uint (`readH`); int8 = per-row-scale q8 4-per-uint (`readQ8`) + `.scales`
   sibling — matmuls only; norms/embeddings/heads ALWAYS fp16. Embeddings sharded ×16.
4. **Kernels**: one `.compute` per family, kernels follow the cache-index/SetBuffer/ceil-dispatch
   idiom; every shader registered in `Main/DeepUnityMeta.cs` (lazy Resources.Load) — shared file,
   main-session edits only.
5. **Frame discipline**: everything is a coroutine yielding under budgets — weight upload
   (bytes/frame), kernel warmup (one/frame at Prewarm), inference (yield per token/layer; sync
   vs async readback samplers). GPU work interleaves across models (LLM + TTS same frame).
5b. **GPU-only runtime** (user directive 0711): models never ship a CPU inference backend —
   CPU implementations (e.g. KokoroCPU) exist strictly as validation oracles for kernel/parity
   grading. Sole exception: microsecond-scale sequential cells (tiny LSTMs in Kokoro's
   predictors, Parakeet's TDT decode loop) run CPU-side inside the GPU pipeline where dispatch
   overhead would dominate — a documented hybrid boundary per model, not a fallback.
6. **Validation**: every port ships a parity harness — Python `dump_reference.py` per-stage .npy
   (fixed seeds/noise exported as tensors when possible) → Unity probe compares corr >0.99 →
   headless batch runner (exit-code + done-marker) → ProbeLogs/ report + listen/QA probes.
7. **Licensing**: everything portable & GitHub-safe (Apache-2.0/MIT/CC0/CC-BY only; espeak-GPL
   rejected); weights folders gitignored, regenerated via import_params.py. THIRD_PARTY_NOTICES
   pending (task #12).

### Conversation compaction (LLM family)

`LLM.Compact(system_prompt, onSummary, max_summary_tokens)` — **virtual on the LLM base** (0711,
reshaped 0714 per user spec): the model answers a bare `"Compact the conversation."` user prompt
with a single-shot compact of the whole history (greedy; no elaborate instructions — the model is
expected/finetuned to know), then the chat re-initializes as [system prompt + `HISTORY:` compact],
shrinking the KV cache while preserving what mattered. Coroutine — runs behind gameplay between
turns. Overridable per model (token-level history splicing, keep-last-K-turns).
**Roadmap idea (dissertation-adjacent):** a small FINETUNED background compactor — a dedicated
tiny model (or LoRA of the chat model) trained specifically to compress game dialogues, running
as a low-budget background coroutine while the main model keeps talking, à la Claude Code's
background compaction. Not scheduled yet; capture data (dialogue → good-summary pairs) once the
demos are live.

### NPC-AI systems roadmap (post-CosyVoice3; shared philosophy: heavy work hides where the
### player isn't looking — same thread as SlowPrefetch/Defetch)

- **AutoCompact** — LLM watches its own token count vs maxModelLength, triggers Compact()
  between turns at a threshold. Cheap; lands with WS-F.
- **SaveMemory()/LoadMemory()** — persist the compacted summary per NPC to disk; next session's
  InitializeChat injects it as [Past conversation] → cross-session NPC memory. Cheap; WS-F.
- **SnapshotConversation()/RestoreConversation()** — full conversation state (history + KV via
  the existing disk-cache machinery) as savegame data; dialogues resume exactly.
- **Type-ahead prefill** — prefill the player's draft text incrementally WHILE they type; on
  send only the delta prefills → near-instant reply start (perceived-latency killer).
- **Background compaction v2** — compaction runs only while the NPC is idle, abortable via the
  epoch pattern if the player engages, atomic history swap on completion; later upgraded by the
  finetuned compactor above.

## Model pool (2026-07-12)

| Model | Family | Status |
|---|---|---|
| Qwen3.5 0.8B/2B | LLM | ✅ shipped — fp16/int8/int4, KV fp16/int8, benchmarked 4060+1650 |
| Gemma3 270M | LLM | ✅ shipped |
| MiniCPM5 1B | LLM | ✅ shipped |
| Chatterbox-Turbo 0.5B | TTS | ✅ shipped — parity PASS; clause-streaming; superseded by CosyVoice3/Kokoro for RT |
| Kokoro-82M v1.0 | TTS | ✅ shipped — (superseded as default by pocket-tts); 29/29 kernels corr 1.000000; int8; GPU kernel speedup (vocoder ~4×, **RTF 0.15–0.30**); 0 frame drops speaking; CPU-LSTM predictor is the only non-GPU stage (~30ms in IL2CPP) |
| CosyVoice3-0.5B | TTS | ✅ shipped — full A0→A7 + A6-MAX campaign: **streaming RTF 2.90→1.15** (int8), LM 151 tok/s, TTFA 2.49s, offline RTF 1.03, parity corr 0.9999 argmax MATCH, seams 4/4 ≤ nat. Voice cloning offline (make_voice.py). Phase 7 (batch readback → RTF<1) optional |
| **pocket-tts (Kyutai) ~100M** | TTS | ✅ **DEFAULT NPC TTS (#27)**. FlowLM (SentencePiece cond → 6L RoPE causal transformer → SimpleMLPAdaLN 1-Euler flow head, ldim32) + Mimi codec (SEANet + 2L decoder-transformer, 250-key window) → 24kHz; **voice cloning** (audio_prompt prefix). **P1–P5 bit-exact vs PyTorch (fp16 corr 1.0 / wav 0.999999)**; KV-cache decode **10× speedup**, **RTF 0.15–0.18 offline / 0.31 streaming fp16 · 0.15/0.34 int8**, TTFA ~100–335ms, streamed==offline bit-identical (0 clicks/underflows). **int8 shipped** (103 MB, −46%; per-stage ≥0.998, mel-corr 0.925 vs fp16 — user-accepted 2026-07-13). **C# SentencePiece encoder** (P7, ids exact-match) → Say(string) Python-free; TtsModel registry + default switched. Names pronounced correctly (user QA). P8 IN PROGRESS: runtime voice-clone cache (Mimi encoder → CloneVoice(AudioClip) → speaker_proj → audio_prompt, disk-cached by audio hash) |
| Qwen3-ASR 0.6B/1.7B | STT | ✅ shipped — **GPU validated 6/6 EXACT** (RTF 0.22–0.66); QwenASRSTT:STT |
| Parakeet-TDT 0.6B v2/v3 | STT | ✅ shipped — **GPU validated 6/6 EXACT** (RTF 0.08–0.09); ParakeetSTT:STT (v3 = Romanian) |

Cross-cutting DONE: ModelBase/TTS/STT bases · residency lifecycle · **F2 tokenizer hierarchy
(TokenizerBase)** · **F3 folder move under InferenceEngine/** · **player-build shipping (2026-07-13):
weights/tokenizers are File-IO from `Assets/...` (editor-only paths) → `DeepUnityMeta.ResolvePath`
redirects to `<Game>_Data/StreamingAssets/<same tail>` in players, `Main/Editor/DeepUnityBuildStep.cs`
(IPostprocessBuildWithReport) copies Weights + tokenizer/G2P/preset files into the build output
(NOT into Assets/StreamingAssets — keeps GBs out of the import pipeline); every *Weights/tokenizer
ctor + res-vs-legacy picker wraps its path; Gemma3 system-prompt KV cache → persistentDataPath in
players** · **GPU-teardown safety (2026-07-13): finalizers marshal ComputeBuffer.Release to the
main thread via DeepUnityDispatcher.RunOnMainThread (finalizer-thread release = native crash in
players); live-model sweeps release everything on Application.quitting, ExitingPlayMode AND
beforeAssemblyReload (mid-play recompile orphaned buffers — the "Leak Detected: Persistent 336"
report), hooks registered via InitializeOnLoadMethod so they survive domain reloads; PocketTTS is
disposed by PocketTTSVoice's hook since it's not yet under ModelBase (WS-F)** ·
**InferencePerf.cs (2026-07-13): ALL cross-engine GPU tuning knobs centralized as documented
mutable statics (arbiter thresholds, silent-refill turbo, tick-calibration band, escalation caps)
with low-end/high-end directions; PocketTTSVoice self-adapts on weak GPUs — underrun re-gate,
prebuffer 0.5→3s, chunk 8→16 escalation, and the REVERSE #29 arbiter (Qwen holds token bursts
while a voice is silent-but-pending, hard-capped for liveness) — validated interactively on a
GTX 1650 (offline RTF int8 0.57 / fp16 0.64 via the new dump-free PocketTTSRtfProbe)** ·
unified import_params pool ·
LLMRegistry (auto-extending NPC picker) · NPCChatBase (2D+3D) + LLMPool + prefetch zones + KV disk
persistence · demos ChatDemo3D / ChatDemo2D (farm) / ForestFork / VoiceLab (all E2E green).

**NPC stack reference:** `Assets/DeepUnity/NPC/ARCHITECTURE.md` — system map, lifecycle, and
step-by-step tweak recipes (integrate a fine-tuned LLM via `[LLMEntry]`, add NPCs/voices/windows,
which perf knob for which symptom, invariants). Read it BEFORE modifying the NPC/LLM/TTS wiring.

## Task board (2026-07-12) — source of truth, update as tasks land

**Done:** A0–A7 CosyVoice3 · Kokoro port + speedup · STT Qwen-ASR + Parakeet (GPU-validated) ·
import_params registry · THIRD_PARTY_NOTICES · NPCChatBase · dissertation TeX · chat thinking-dots +
think-filter · **F2 tokenizer hierarchy** · **F3 InferenceEngine restructure** · chat text-jitter fix ·
Kokoro question-prosody probe.

**Remaining (priority order):**

| # | Task | Priority |
|---|---|---|
| ~~20~~ | ✅ **DONE** Qwen3.5 decode frame-pacing — VALIDATED headless 2026-07-13 (`LMFramePacingProbe`, 3-arm A/B, 4060 int8): pre-#20 spread+sync = a 20-30 ms hitch EVERY token; shipped burst+`SampleYielding` = **0 frames >20 ms in 9608, p95 1.65 ms, max 15.9 ms, tok/s unchanged (30.8)**. Results in BENCHMARK.md; `DebugSpreadDecode` = A/B toggle | ✅ done |
| ~~27~~ | ✅ **DONE** pocket-tts GPU port → DEFAULT NPC TTS. P1–P8 all validated: fp16 bit-exact, int8 shipped (mel-gate, user-accepted), C# SentencePiece exact, registry + default set, names QA'd, voice-clone cache (Mimi encoder → CloneVoice(AudioClip) corr 1.0; 3 tiers: Resources bake / persistent / encode; **inspector precompute button** on NPCChatBase; ref cropped pause-aware near the 10 s native prompt len — never mid-word). **Long-reply safe**: 2D dispatch guard (>65535 groups) + windowed streaming decode (CTX 40, O(1)/chunk, corr 1.0 vs direct). **Integration polish live-QA'd**: drain-grace pause (last words audible), OnClauseSpoken audio-synced text reveal, spread prefill + async readbacks (no Interact hitch). ChatDemo3D: Velmire → PocketTTS/jean + user-cloned voices tested | ✅ done |
| ~~29~~ | ✅ **DONE 2026-07-13** Talk-time frame drops — FramePacing arbiter (Qwen `NoteLlmIssue` ↔ TTS pump deferral at ring ≥ headroom) + `elem_offset` row-sliced kernels + self-calibrating `PocketTTS.GpuMacsPerTick` (3-7 ms heavy-tick target, never-shrink-while-ring-low guard). Talk-time frames >22 ms 222→54, >33 ms 39→16, max 116→69 ms; parity P1/P4/P5/P8b re-PASS bit-exact; probe = `NpcTalkPerfProbe`. Shipped in `ed0c06fc`. Last un-pulled lever (fully GPU-resident AR loop) parked in session task #30 | ✅ done |
| ~~24~~ | ❌ **DROPPED 2026-07-13** A6-MAX CosyVoice3 sub-1.0 RTF — user: pocket-tts is the permanent TTS standard, CosyVoice3 optimization abandoned (port stays in repo as-is, streaming RTF 1.71 final) | ❌ dropped |
| ~~28~~ | ✅ **DONE 2026-07-13** Background KV compaction + `HistoryMode.ResumeFromCompact` ACTIVE (clamp removed). Close-time maintenance in NPCChatBase: past `compactionTriggerTokens` (est. chars/4, default 512) a clean close runs `CompactConversationRoutine` on the resident idle model — full-state KV save first (crash-safe fallback) → `LLM.Compact` (greedy self-briefing, now takes `post_summary_context`) re-seeds `[system + summary + last 2 verbatim turns]` → transcript trims, briefing rides in the transcript JSON (`TranscriptState.summary`) → compacted KV saved (overwrites). Superseded by #33's v2 semantics (2026-07-14) — see #33. NOT YET play-tested | ✅ done (superseded by #33) |
| 30 | **pocket-tts DECODER OPTIMIZATION** — phase 1+2 DONE 2026-07-13 on the 1650 box: (1) groupshared-tiled `Conv1DTiled`/`ConvTranspose1DTiled` (input window staged once per 8-row tile, weights reused 8×; legacy Conv1D re-read X per output channel = 512× traffic at conv0 which alone was 43% of decode) + `CausalAttention` pass-3 rewrite (per-tile softmax weights in shared; old pass recomputed every q·k dot in all 64 lanes) — **all BIT-exact** (accumulation order preserved; `PocketTTSKernelParityProbe` gates tiled-vs-legacy via `ForceLegacyKernels`, no dumps needed). (2) tail-restricted window decode: `Decode/DecodeYielding(tailLatents)` walks the causal receptive field backwards (SEANet margins + per-layer 250-row attn margins) and computes ONLY the kept tail — streaming/windowed callers pass their chunk; kept samples bit-exact (probe gate 2). **Results GTX 1650 int8: offline decode 4004→865 ms, RTF 0.584→0.288; streaming window (CTX40+12) 990→125 ms (~8×)**. Voice PlayerPrefs keys bumped to .v2 (old 3s-prebuffer learnings stale). Remaining levers if ever needed: FFN LinearBiasQ8 X-row tiling (~22% of decode), GPU-resident AR loop — parked; **flow AR loop (1.8 s/10.4 s) is now the bottleneck → see #31**. **4060 A/B 2026-07-14: decoder 1.77× (452→256 ms), RTF 0.143→0.123 — cross-GPU confirmed (BENCHMARK.md §2026-07-14; `LegacyKernelABRunner` menus = the A/B on any box)** | ✅ phases 1-2 done (play QA pending) |
| 31 | **Qwen3.5 GEMV/GEMM optimization** — DONE 2026-07-13 on the 1650 box. Profiled per-token decode (`QwenDecodeProfileProbe`, edit-mode sync via `Qwen3_5Model.LoadBlockingForProbe` — no play mode): the legacy 1Vec kernels gave each output row ONE thread striding its weight row (fp16 LM head = 41% of the token, ~30-60x bandwidth waste). Fix: warp-per-row coalesced kernels — `*1VecCoal` decode GEMVs (X staged in groupshared, lanes read consecutive packed words, tree-reduce) + `*GemmCoal` prefill GEMMs (8-token tile staged per group, every weight read reused 8x). **Results GTX 1650 int8: decode 8.2 -> 40.9 tok/s (121 -> 24.5 ms/tok), prefill(64) 1725 -> 484 ms; parity corr 1.000000000 + argmax match** (`GEMV Parity (coal vs legacy)` menu — logits-level gate, lane reduction reorders float sums so bit-exact is impossible by design; `ForceLegacyGemv` keeps the legacy path alive). GOTCHA that cost an hour: the routing ternaries MUST keep the legacy `vec1` branch (`coalMlp ? new : gemmMlp ? gemm : vec1 ? kGateUp1 : kGateUp`) — dropping it dispatched the batch kernel with 1-vec group counts in the legacy pass -> stale intermediate rows -> session-dependent garbage that looked like a new-kernel bug. Untouched levers if ever needed: glue copies/norms fusion (~7%), DeltaNet scan occupancy (16 groups), full-attn projections. **4060 A/B 2026-07-14: decode 29.6→70.0 tok/s (2.37×), prefill-64 1.64×, parity PASS corr 1.0 (BENCHMARK.md §2026-07-14)** | ✅ done (play QA pending) |
| ~~32~~ | ✅ **DONE 2026-07-14** InferencePerf measured AutoTune — sync-vs-async decode is now a per-session MEASURED decision, not a universal constant (v0.14.7 always-sync put the 4060's ~14 ms token stall into every reply frame — undid #20's smoothness on GPUs that didn't need the throughput). First 6 sync tokens probe (median, timed in `Qwen3_5.DecodeStep`) → SYNC if tokenMs ≤ `SyncStallBudgetMs` (packing = floor(budget/tokenMs) tok/frame), else ASYNC #20 path unless est. async tok/s < `MinUsableTokS` (weak-GPU fallback → sync). ONE user dial: `InferencePerfTuner.replySpeedBias` scene slider (0 smooth ⇄ 1 fast; scales the budget, re-probes live; on the `InferencePerf` object in all 3 demo scenes). PocketTTSVoice prefs v3: GPU-keyed + clean-session walk-back (one rung down after a zero-underrun session) — nothing learned on one GPU/one contended session can degrade another. NOT YET play-tested | ✅ done (play QA pending) |
| ~~33~~ | ✅ **DONE 2026-07-14** ResumeFromCompact v2 (user spec, replaces #28's cancel semantics). (1) `LLM.Compact` reshaped: bare `"Compact the conversation."` user prompt (model expected/finetuned to answer with the full compact in ONE message — no elaborate instructions), re-init as `[system + "HISTORY:\n" + compact]`, `post_summary_context` param removed. (2) NPCChatBase: transcript CLEARS after compaction (the HISTORY block IS the history; `RECENT_TURNS_KEPT` deleted), `BuildResumePrompt` = system + HISTORY + turns-since-compact, tier-c `resume` also fires on compact-only (empty-transcript) states. (3) **Never canceled once started**: zone-exit `ReleaseLlmAfterKvSave` WAITS on `compactRoutine` + `kvSaveInFlight` (model stays on GPU until compact + KV snapshot land; skips release if the player walked back into the zone during the wait); dialogue reopen WAITS in `OpenConversation` — sends are blocked (state still PreparingForInteraction) and the thinking-dots bubble pulses **"Compacting…"** until it lands; `CancelCompaction` deleted (only the pre-touch guard can still bail). **Mechanical probe ALL PASS 2026-07-14** (`NpcCompactProbe` + `DeepUnity/NPC/Run Compact Probe` menu, report `ProbeLogs/npc_compact_probe.md`): compact starts on close, reopen blocked+waits (+1.7s after compact), transcript cleared, KV re-saved, zone-exit held the model through compaction and released +3.3s after; compaction-behind-gameplay frames ≈ talk-time (mean ~12.5 ms vsync-off). Probe gotcha: play-mode entry domain-reloads → runner re-arms via SessionState + InitializeOnLoadMethod. User play QA pending | ✅ done (probe PASS) |
| ~~34~~ | ✅ **DONE 2026-07-15** Mid-reply interruption (send-while-talking / leave-while-talking). Root bug: AskNPC during the speech tail hard-cut the audio, and Escape mid-reply `StopCoroutine`'d the Chat coroutine (could land mid-forward → half-written KV → chatLive dead). Fix: `LLM.CancelChat()` in the base — cooperative flag; all 3 decode loops (Qwen3.5/Gemma3/MiniCPM5) exit at the NEXT TOKEN BOUNDARY exactly like a natural stop token (KV keeps the truncated turn, the next Chat closes it with the template suffix as usual; flag cleared at Chat entry). NPCChatBase: AskNPC now also fires during Talking/voice-tail → `InterruptThenAsk` (cancel → `FadeOutVoices` ~1 s → wait unwind + quiet → new Talk lands the question); `CloseInteraction` mid-reply cancels cooperatively and `CloseConversationWhenReplyUnwinds` runs a CLEAN close (chatLive stays true, KV save + resume eligible; hard StopCoroutine only as a 10 s dead-model fallback); Talk tail gated by `stillOpen`/`replyCanceled` (no state resurrection after a close, no voice flush of a canceled reply, OnReplyFinished skipped); a running compaction is never canceled (guards in both paths). **User-repro fix (same day)**: play QA caught "full text pops + audio keeps playing" — `VoicesAudible` used `IsSpeaking` only, which is FALSE in the drain window (synthesis done, ring/tail still audible — seconds long at 6x-realtime synth) → the ask took the direct path and talked over the ring. Fix: audibility = `IsSpeaking || IsAudioPlaying` (VoicesAudible + PrepareForNextReply stop-guards), and `InterruptThenAsk` now freezes the bubble at what was actually SPOKEN (kills the typewriter + `revealActive=false` up front, so no settle path reveals unspoken text; dots bubble popped if nothing spoke; hard `StopVoices` insurance after the fade deadline). **Probe ALL PASS 2026-07-15** (`NpcInterruptProbe`, menu `DeepUnity/NPC/Run Interrupt Probe (ChatDemo3D)`, report `ProbeLogs/npc_interrupt_probe.md`), incl. the new asserts "old audio FULLY quiet (incl. ring/tail) BEFORE the new turn landed" on both the mid-generation and the speak-tail ask: cancel +0.05 s, truncated turn recorded, follow-up coherent, close-mid-reply unwinds with chatLive TRUE + KV save, reopen resumes, 0 console errors. User play QA pending | ✅ done (probe PASS) |
| ~~35~~ | ✅ **DONE 2026-07-15** THE COMPACTION STANDARD (user spec) + interrupt round-2 fixes. Compaction timing: the limit-hitting reply is delivered IN FULL (decoded + typed + SPOKEN; +8192 KV headroom absorbs the overshoot) — Talk's tail now WAITS for `VoicesAudible()/revealJob/revealQueue` before starting `CompactConversationRoutine`; "Compacting…" only after the voice quiets; stands down if a new ask took over (re-triggers after ITS reply) or the dialogue closed (next open's crash-recovery). Window: keeps the whole conversation through compaction; `RepopulateWindow` on the next open renders the compacted past as ONE dimmed block (`ThinkStyled(compactSummary)`) + turns since. Round-2 interrupt fixes from play QA: `VoicesAudible = IsSpeaking OR IsAudioPlaying` (IsSpeaking alone missed the seconds-long drain window → ask talked over the ring); hard-stop now DESTROYS the tainted stream clip (Unity's PCM reader pre-buffers ~0.2-0.8s of old speech that survived Pause — the "1s of previous reply" bug) in PocketTTS+Kokoro voices; reveal machinery dies at close (leaked revealActive/pendingFullReply resurrected the old reply's text at the next dialogue's first send, even on ResetEveryTime); caret re-activates after every send; `CloseConversation` KV-save skipped while a compaction forwards the model (pre-existing tear race). Docs: standard written into ARCHITECTURE.md §2/§R4 + ResumeFromCompact tooltip + CompactConversationRoutine header. **Probes: NpcCompactProbe 22/22 (incl. A0 "no compaction while speaking") + NpcInterruptProbe 24/24, 0 console errors.** Probe gotcha: scene Velmire has maxContextLength=128 (user's compaction testing) — probes force their own limits. Fable review agent audit pending | ✅ done (probes PASS) |
| ~~36~~ | ✅ **DONE 2026-07-15 (Fable agent)** NPC hardening round 2 — remaining audit findings: per-LLM `kvSavesInFlight` dict replaces the global static save gate (#9, both probes rebound to the dictionary read), Kokoro residency holder-refcount mirroring pocket (#11), prefill-cancel in all 3 Chat impls (#14 — prefill always completes, generation skipped, empty turn closes as usual), `LLM.Busy` guard: public non-virtual wrappers over protected *Core methods for Chat/InitializeChat/Compact/SaveConversationKV/TryRestoreConversationKV, warn+refuse on re-entry, CompactCore drives Cores directly (F1), `OnDestroy` pool-ref release (F5), dead-comment sweep (#16). **Agent-run Unity verification: compile clean; Compact 22/22 + Interrupt 24/24 + NEW `NpcAuditProbe` 23/23** (idle-sibling Leave no-ops with bait state, AskNPCSilent dropped-with-warning mid-reply + voice tail, Busy latched during generation with ZERO backstop fires, cancel-in-prefill closes clean +0.61s with the new prefill-cancel log line confirmed). Not probe-covered by design: Kokoro refcount (needs 2D two-NPC scene; mirrors proven pattern), OnDestroy release (teardown path implicitly exercised). Deliberately left: Generate() unguarded (benchmark-only), Kokoro PrefetchNow still no BeginLoad restart after full defetch (pre-existing) | ✅ done (3 probes PASS) |
| 25 | Paper benchmark matrix (RTF/TTFA/load/VRAM, all engines × quant) on 4060 (+1650 numbers now exist via PocketTTSRtfProbe) | unblocked |
| 15 | WS-F — LLM↔ModelBase unification (needs the 3 LLM loaders rewritten to the epoch/BeginLoad/Defetch contract — RISKY, touches the validated load path; also lets PocketTTS join the ModelBase teardown sweeps instead of its own hook) | deferred |
| 18 | WS-G rest — NPC-AI primitives (per-NPC memory, conversation snapshots, type-ahead prefill, mid-conversation AutoCompact) | backlog |

Done 2026-07-13 (audio-on-weak-GPUs groundwork for #30): InferencePerf universal-standard constants
(nobody hand-tweaks per GPU) · adaptive prebuffer/chunk escalation with per-device PlayerPrefs
persistence (learns once, keeps forever; pure audio-side — prefetch zones/pool/KV untouched) ·
#29 reverse arbiter (Qwen holds token bursts during audible silence, hard-capped) · silent-refill
turbo · leave-fade (FadeOutAndStop).

(Full per-task history + gotchas live in the session memory `project_deepunity_cosyvoice3_port.md`.)
