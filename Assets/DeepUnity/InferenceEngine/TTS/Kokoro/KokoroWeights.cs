...
namespace DeepUnity
{
    ...
    {
        // Manifest-driven weight store for the Kokoro-82M export (fp16 .bin files + manifest.tsv
        // written by validation/import_kokoro.py). Same TSV schema as Chatterbox/CosyVoice.
        //
        // Implements the ModelBase RESIDENCY contract loader-side (pattern copied from
        // CosyVoiceWeights — keep the two in sync when the contract evolves):
        ...
        //
        // CLEANUP INVARIANT (mirrors CosyVoiceWeights): once a job is dequeued, its pooled byte[]
        // and its IO-gate slot are OWED cleanup on EVERY exit path of the pump — success, abort,
        // stale-epoch — otherwise a mid-job Defetch strands a gate slot and DefetchPump's drain
        // waits forever.
        //
        // Kokoro-specific: the fp16 export is all-f16; the int8 export (weights_kokoro_int8,
        // import_kokoro.py --quant int8) adds dtype "q8" rows (int8 packed 4-per-uint, KokoroCS
        ...
        public class KokoroWeights : IDisposable
        {
            ...
            struct Entry
            {
                ...
                public string dtype;           // "f16" (only dtype in the kokoro export)
                public int numel;
                public int[] shape;
                public string file;
            }
            ...
            public KokoroWeights(string paramsPath, bool beginLoad = true)
            {
                ...
                if (!Directory.Exists(paramsPath))
                    throw new DirectoryNotFoundException(
                        ...
                        "Assets/DeepUnity/InferenceEngine/TTS/Kokoro/validation/import_kokoro.py — it exports to " +
                        "Assets/Resources/Weights/weights_kokoro_fp16/.");
                _root = paramsPath;

                string manifest = Path.Combine(paramsPath, "manifest.tsv");
                if (!File.Exists(manifest))
                    throw new FileNotFoundException($"manifest.tsv missing in {paramsPath} (re-run import_kokoro.py).");
            ...
            /// instant release; &gt; 0 → buffers released across frames under that budget.</summary>
            ...
            IEnumerator DefetchPump(long slowBytesPerFrame)
            {
                ...
                if (_reloadAfterDefetch && !_disposed)   // "player came right back"
                {
            ...
            /// <summary>GPU buffer of an exported tensor (manifest name, e.g. "bert/layer/attn_q.w").
            /// fp16 packed 2-per-uint (readH). Null until its upload completed.</summary>
            ...
            /// <summary>Synchronous CPU read of a tensor's file, fp16 widened to fp32 (voicepacks,
            /// LSTM weights for the CPU path).</summary>
            ...
            IEnumerator UploadPump(int epoch)
            {
                ...
                if (_jobsUploaded != _jobsTotal)
                    ConsoleMessage.Warning($"Kokoro weights: only {_jobsUploaded}/{_jobsTotal} tensors uploaded " +
