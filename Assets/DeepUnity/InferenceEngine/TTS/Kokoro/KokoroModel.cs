...
namespace DeepUnity
{
    namespace KokoroModeling
    {
        ...
        public class KokoroModel : IDisposable
        {
            ...
            /// tenc channels of [C,T] -> (posStride 1, chStride T). eps 1e-12 / 1e-5.</summary>
                                  ...
                                  int C, float eps, int posStride, int chStride)
            {
                ...
                cs.SetBuffer(kLN, "norm_input", x);
                cs.SetBuffer(kLN, "norm_output", y);
                cs.SetBuffer(kLN, "ln_gamma", weights.Get(name + ".w"));
                cs.SetBuffer(kLN, "ln_beta", weights.Get(name + ".b"));
                cs.Dispatch(kLN, Div256(positions), 1, 1);
            }
            ...
            /// register-blocked Conv1DTile (raw-X prologue); the rest (strided F0/N_conv,
            /// noise_conv0 k12s6, 1-channel projs) keep the naive per-output kernel.</summary>
                             ...
                             int Cout, int Tout, int k, int stride, int pad, int dil, bool bias = true)
            {
                ...
                if (tile)
                {
                    cs.SetInt("conv_in_mode", 0);          // raw X; bind never-read prologue slots
            ...
            /// residue -> no k-loop divergence, coalesced X) — bit-exact same math.</summary>
                                               ...
                                               Action<float[]> onWav)
            {
                ...
                // ---------------- TextEncoder (GPU convs + CPU biLSTM) --------------------------
