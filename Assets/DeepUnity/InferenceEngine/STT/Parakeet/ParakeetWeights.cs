...
namespace DeepUnity
{
    ...
    {
        // Manifest-driven GPU weight store for the Parakeet-TDT export (fp16/int8 .bin files +
        // manifest.tsv written by validation/import_parakeet.py).
        //
        // Implements the ModelBase RESIDENCY contract loader-side (verbatim CosyVoiceWeights
        // pattern):
        ...
        //
        // Parakeet delta: the TDT decode side runs on the CPU (SPEC §8), so tensors under
        // dec/*, joint/head.* and frontend/* are NOT uploaded — ParakeetCPU reads them via
        // ParakeetTensors. Only the encoder (+ joint/enc_proj) becomes GPU-resident.
        //
        ...
        public class ParakeetWeights : IDisposable
        {
            ...
            /// <summary>Total/resident GPU weight bytes for ModelBase.LoadProgress.</summary>
            public long BytesTotal { get; private set; }
            public long BytesUploaded { get; private set; }

            const int MAX_IO_JOBS = 4;
            static readonly string[] CPU_SIDE_PREFIXES = { "dec/", "joint/head", "frontend/" };
            ...
            public ParakeetWeights(string paramsPath, bool beginLoad = true)
            {
                ...
                if (!Directory.Exists(paramsPath))
                    throw new DirectoryNotFoundException(
                        ...
                        "Assets/DeepUnity/InferenceEngine/STT/Parakeet/validation/import_parakeet.py — it exports " +
                        "under Assets/Resources/Weights/.");
                _root = paramsPath;
                ...
                foreach (string line in File.ReadAllLines(manifest))
                {
                    ...
                    bool cpuSide = false;
                    for (int i = 0; i < CPU_SIDE_PREFIXES.Length; i++)
                        if (p[0].StartsWith(CPU_SIDE_PREFIXES[i], StringComparison.Ordinal)) cpuSide = true;
                    if (cpuSide) continue;     // read CPU-side by ParakeetTensors, never uploaded
            ...
            IEnumerator DefetchPump(long slowBytesPerFrame)
            {
                ...
                ConsoleMessage.Info("Parakeet-TDT weights defetched from GPU.");
            ...
            /// <summary>GPU buffer of an exported tensor (manifest name, e.g. "layer_0/attn.q.w").
            /// Null until its upload completed. CPU-side tensors (dec/*, joint/head.*, frontend/*)
            /// are NOT here — read those via ParakeetTensors.</summary>
            public ComputeBuffer Get(string name)
            {
                if (!_entries.TryGetValue(name, out Entry e))
                    throw new KeyNotFoundException($"Parakeet GPU store has no tensor '{name}' " +
                        "(CPU-side tensors live in ParakeetTensors).");
                return e.slot[0];
            }
            ...
            IEnumerator UploadPump(int epoch)
            {
                ...
                if (_jobsUploaded != _jobsTotal)
                    ConsoleMessage.Warning($"Parakeet weights: only {_jobsUploaded}/{_jobsTotal} tensors uploaded " +
                                           "(missing or failed reads — see earlier exceptions). Output will be invalid.");
                else
                    ConsoleMessage.Info("Parakeet-TDT encoder weights streamed to GPU.");
