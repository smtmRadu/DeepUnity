...
namespace DeepUnity
{
    ...
    {
        // Manifest-driven weight store for the Qwen3-ASR export (fp16/int8 .bin files +
        // manifest.tsv written by validation/import_qwen3asr.py).
        //
        // VERBATIM CosyVoiceWeights residency-lifecycle pattern (TTS/CosyVoice/CosyVoiceWeights.cs)
        // — loader-side implementation of the ModelBase RESIDENCY contract:
        ...
        public class QwenASRWeights : IDisposable
        {
            ...
            public QwenASRWeights(string paramsPath, bool beginLoad = true)
            {
                ...
                if (!Directory.Exists(paramsPath))
                    throw new DirectoryNotFoundException(
                        ...
                        "Assets/DeepUnity/InferenceEngine/STT/QwenASR/validation/import_qwen3asr.py — " +
                        "`python import_qwen3asr.py <Qwen3-ASR-*-hf checkpoint dir>` exports it " +
            ...
            IEnumerator DefetchPump(long slowBytesPerFrame)
            {
                ...
                ConsoleMessage.Info("QwenASR weights defetched from GPU.");
            ...
            /// "dec/layer_0/q_proj"). Null until its upload completed.</summary>
            ...
            IEnumerator UploadPump(int epoch)
            {
                ...
                if (_jobsUploaded != _jobsTotal)
                    ConsoleMessage.Warning($"QwenASR weights: only {_jobsUploaded}/{_jobsTotal} tensors uploaded " +
                                           "(missing or failed reads — see earlier exceptions). Output will be invalid.");
                else
                    ConsoleMessage.Info("Qwen3-ASR weights streamed to GPU.");
