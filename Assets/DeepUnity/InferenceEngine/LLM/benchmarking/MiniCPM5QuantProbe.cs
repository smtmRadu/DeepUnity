...
namespace DeepUnity
{
    // A/B probe for weight-only INT8 vs FP16 on MiniCPM5-1B (the MiniCPM5 twin of QuantProbe).
    ...
    // difference is visible as text. Driven via FlashAttnProbeRunner.RunMiniCPMInt8; status ->
    ...
    public class MiniCPM5QuantProbe : MonoBehaviour
    {
        ...
        void Status(string s)
        {
            ...
            Debug.Log("[MiniCPM5QuantProbe] " + s);
        }
        ...
        static void Prefill(MiniCPM5ForCausalLM lm, float[] ids)
        {
            lm.model.ResetCache();
        ...
        IEnumerator Exercise(MiniCPM5ForCausalLM lm, float[] prompt, int[] feedTokens,
                             float[][] logitsOut, int[] ownTokens, double[] medianMsOut, string[] textOut, string label)
        {
            int V = MiniCPM5Modeling.MiniCPM5Config.VOCAB_SIZE;
            var w = lm.Warmup();
        ...
        IEnumerator Run()
        {
            ...
            report.AppendLine($"# {quant} vs FP16 probe (minicpm5-1B)");
            report.AppendLine();
            ...
            // ---------------- FP16 reference ----------------
            Status("constructing FP16 MiniCPM5");
            var fp16 = new MiniCPM5ForCausalLM(LLMQuant.FP16, kv_quant: KVQuant.FP16);
            ...
            // ---------------- quantized ----------------
            Status($"constructing {quant} MiniCPM5");
            var int8 = new MiniCPM5ForCausalLM(quant, kv_quant: kvForQuant);
        ...
        void WriteReport(bool success)
        {
            try
            {
                ...
                Debug.Log($"[MiniCPM5QuantProbe] report written to {reportDirectory}");
            }
            catch (Exception e)
            {
