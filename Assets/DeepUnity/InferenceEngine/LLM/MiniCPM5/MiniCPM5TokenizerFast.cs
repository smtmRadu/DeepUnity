...
namespace DeepUnity
{
    // Byte-level BPE tokenizer for MiniCPM5 (llama-family checkpoint, GPT-2 style byte-level BPE). Parses HuggingFace tokenizers `tokenizer.json`
    // directly (no preprocessing).
    //
    // Pipeline (Encode):
    //   1. Locate special tokens in the input via regex; tokenize the surrounding text via BPE.
    //   2. Pre-tokenize each plain-text segment with the GPT-2-family regex (MiniCPM splits digit runs in groups of up to 3).
    ...
    public class MiniCPM5TokenizerFast : TokenizerBase
    {
        public static readonly int ENDOFTEXT_TOKEN_ID = MiniCPM5Modeling.MiniCPM5Config.ENDOFTEXT_TOKEN_ID;
        public static readonly int IM_START_TOKEN_ID  = MiniCPM5Modeling.MiniCPM5Config.IM_START_TOKEN_ID;
        public static readonly int IM_END_TOKEN_ID    = MiniCPM5Modeling.MiniCPM5Config.IM_END_TOKEN_ID;
        public static readonly int EOS_TOKEN_ID       = MiniCPM5Modeling.MiniCPM5Config.EOS_TOKEN_ID;
        ...
        // Pre-tokenizer regex (GPT-2 family). MiniCPM5's tokenizer.json splits digit runs in
        // groups of up to 3 (\p{N}{1,3}) — the only difference from the Qwen/GPT-2 pattern.
        // NB: .NET supports \p{L}, \p{N}, \p{M}.
        ...
        public MiniCPM5TokenizerFast(string path = "Assets/DeepUnity/InferenceEngine/LLM/MiniCPM5/MiniCPM5TokenizerFast.json", bool load_async = false)
        {
            ...
            path = DeepUnityMeta.ResolvePath(path);   // player builds: StreamingAssets
            BuildByteMaps();
            preTokRegex = new Regex(PRETOK_PATTERN, RegexOptions.Compiled);

            if (!File.Exists(path))
                throw new ArgumentException($"MiniCPM5TokenizerFast: vocab file not found at {path}");
        ...
        void LoadSync(string path)
        {
            ...
            BuildSpecialRegex();
            IsReady = true;
            ConsoleMessage.Info($"MiniCPM5 tokenizer loaded ({sw.Elapsed.TotalSeconds:0.00} s, vocab={vocab.Count}, merges={mergeRank.Count}, added={addedTokens.Count})");
        }
        ...
        {
            // MiniCPM5's tokenizer.json (tokenizers >= 0.20 era) stores merges as PAIR ARRAYS:
            //   "merges": [["a","b"], ...]
            // while older files (e.g. Qwen3.5's) use "a b" strings. Handle both — the legacy
            // string path silently parses ZERO merges from the array format, which degrades every
            // word to single-byte tokens (the model then babbles character-level output).
            ...
            while (i < end)
            {
                ...
                if (c == '[')
                {
                    // pair-array format: ["a", "b"]
                    int pairEnd = FindMatchingClose(content, i + 1, '[', ']');
                    int j = i + 1;
                    ...
                    while (j < pairEnd)
                    {
                        if (content[j] == '"')
                        {
                            string s = ReadJsonString(content, j, out int qc);
                            if (a == null) a = s; else if (b == null) b = s;
                            j = qc + 1;
                        }
                        else j++;
                    }
                    if (a != null && b != null) mergeRank[(a, b)] = rank++;
                    i = pairEnd + 1;
                }
                else if (c == '"')
                {
                    // legacy string format: "a b" — first space splits the two halves
                    string s = ReadJsonString(content, i, out int qClose);
                    int sp = s.IndexOf(' ');
                    if (sp > 0 && sp < s.Length - 1)
                        mergeRank[(s.Substring(0, sp), s.Substring(sp + 1))] = rank++;
                    i = qClose + 1;
                }
                else i++;
            }
        ...
        public (Tensor, Tensor) Encode(string input, bool add_special_tokens = true, bool truncation = false, int max_length = 512)
        {
            ...
            if (!IsReady) throw new InvalidOperationException("MiniCPM5 tokenizer not yet loaded. Check IsReady first.");
        ...
        // ChatML per the checkpoint's chat_template.jinja: <s> once at sequence start, then
        // <|im_start|>{role}\n{content}<|im_end|>\n per message. No thinking block —
        // enable_thinking is accepted for API parity and ignored.
        public Tensor ApplyChatTemplate(List<Dictionary<string, string>> messages, bool add_generation_prompt = true, bool enable_thinking = false)
        {
            ...
            ids.Add(MiniCPM5Modeling.MiniCPM5Config.BOS_TOKEN_ID);   // <s>
