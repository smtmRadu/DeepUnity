...
namespace DeepUnity.Tutorials
{
    public class Soccerplayer : Agent
    {
        ...
        [Header("Task - He must throw the ball towards the target with no excuses. Note: Needs normalization")]
        private float currentZpos;
        private Vector3 initialTargPos;
        ...
        {
            base.Awake();
            initialTargPos = target.transform.position;
        ...
        public override void OnEpisodeBegin()
        {
            ...
            target.transform.position = new Vector3(initialTargPos.x + Utils.Random.Range(-3f, 3f), initialTargPos.y, initialTargPos.z);
        }
        // 120 input variables
        public override void CollectObservations(StateVector stateBuffer)
        {
            ...
            // 120 + EXTRA 13
            stateBuffer.AddObservation(ball.transform.rotation);
            stateBuffer.AddObservation((ball.transform.position - handL.transform.position).normalized);
            stateBuffer.AddObservation((ball.transform.position - handR.transform.position).normalized);
            stateBuffer.AddObservation((target.transform.position - transform.position).normalized);
