...
{
    public class Skier : Agent
    {
        [Header("Needs normalization")]
        public float survival_reward = 0.001f;
        public GameObject leftSkiblade;
        public GameObject rightSkiblade;
        ...
        public override void Awake()
        {
            ...
            // 16 body parts
            bodyController.AddBodyPart(leftSkiblade);
            bodyController.AddBodyPart(rightSkiblade);
            ...
            {
                if (
                bodypart.gameObject != leftSkiblade.gameObject &&
                bodypart.gameObject != rightSkiblade.gameObject &&
        ...
        public override void CollectObservations(StateVector stateBuffer)
        {
            ...
            // We will add all velocities relative to the chest

            BodyPart _chest = jdDict[chest];
            Vector3 reference_velocity = _chest.rigidbody.velocity;
            Vector3 reference_angular_velocity = _chest.rigidbody.angularVelocity;

            // 4
            stateBuffer.AddObservation(_chest.rigidbody.rotation);

            BodyPart _ls = jdDict[leftSkiblade];
            BodyPart _rs = jdDict[rightSkiblade];

            // 14
            stateBuffer.AddObservation(_ls.rigidbody.velocity - reference_velocity);
            stateBuffer.AddObservation(_ls.rigidbody.angularVelocity - reference_angular_velocity);
            stateBuffer.AddObservation(_ls.ColliderContact.IsGrounded);
            stateBuffer.AddObservation(_rs.rigidbody.velocity - reference_velocity);
            stateBuffer.AddObservation(_rs.rigidbody.angularVelocity - reference_angular_velocity);
            stateBuffer.AddObservation(_rs.ColliderContact.IsGrounded);
