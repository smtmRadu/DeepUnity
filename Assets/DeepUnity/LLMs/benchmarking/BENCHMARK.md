...
- **Qwen3.5-0.8B** — only the **6 full-attn** layers (idx 3,7,11,15,19,23; the 18 DeltaNet layers cache no KV).
  kv_heads=2, head_dim=256 → `6·2·2·256·4 B` = **24 KB/token**, growing linearly to 8192 ctx.
...
- ⬜ (cleanup) `qwen int4 decode` 1650 cell shows 0.0 (timed out) — re-run with a longer probe timeout for a real number, or annotate.
