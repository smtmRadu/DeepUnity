...
namespace DeepUnity
{
    public class Gemma3GPUFP16ForCausalLM
    {
        private string path;
        public Gemma3GPUFP16Modeling.Gemma3GPUFP16Model model;
        ...
        public Gemma3GPUFP16ForCausalLM(
            string params_path = "Assets/DeepUnity/LLMs/Gemma3GPUFP16/params_it",
            string tokenizer_path = "Assets/DeepUnity/LLMs/Gemma3GPUFP16/Gemma3TokenizerFast.json",
            int cacheCapacity = 8192)
        {
            ...
            model = new Gemma3GPUFP16Modeling.Gemma3GPUFP16Model(params_path, cacheCapacity);
            ConsoleMessage.Info($"Gemma3GPU FP16 model created ({sw.Elapsed.TotalSeconds:0.00} s)");
        }

        ~Gemma3GPUFP16ForCausalLM()
        {
            model?.Dispose();
            ConsoleMessage.Info("Gemma3GPU FP16 released from GPU");
        }
        ...
        /// <summary>
        /// Call after IsReady to verify FP16 weights were loaded correctly.
        /// Reads back the first 8 embedding values and prints them.
        /// Compare against the FP32 Gemma3GPU output to check for data corruption.
        /// </summary>
        public void VerifyWeights()
        {
            // Read back first 4 uints (= 8 halfs) from embedding buffer
        ...
        {
            if (!IsReady)
                throw new Exception("Gemma3GPU FP16 is not ready. Check IsReady first.");
        ...
        public IEnumerator InitializeChat(string system_prompt = "")
        {
            ...
            ConsoleMessage.Info($"Gemma3GPU FP16 system prompt computed ({sw.Elapsed.TotalSeconds:0.00} s).");
            ...
            int max_new_tokens = 128, float temperature = 1f, int top_k = -1, float top_p = 1f, float min_p = 0f)
        {
            ...
            for (int t = 0; t < max_new_tokens - 1; t++)
            {
                ...
                {
                    ConsoleMessage.Info("Gemma3GPU FP16 ended the response.");
                    break;
                }
