...
Assets/DeepUnity/TTS/Chatterbox/ChatterboxWeights.cs: name\tfile\tdtype\tnumel\tshape-csv,
fp16 packed 2-per-uint on the GPU side). SEPARATE from Assets/DeepUnity/LLM/import_params.py
...
Conventions (see ../SPEC.md section 10):
  ...
  - --quant int8: ONLY the GPU LinearBias matmul weights (QUANT_TARGETS below — the names
    ...
    Assets/DeepUnity/LLM/import_params.py quantize_int8/TTSExporter.mat: manifest dtype "q8"
...
def quantize_int8(w):  # fp32 [rows, cols] -> int8 + fp16 per-row scales + (max_err, mean_err)
    # copied from Assets/DeepUnity/LLM/import_params.py (keep byte-identical behavior)
...
def main():
    ...
    # Assets/DeepUnity/TTS/Kokoro/validation -> Assets/Resources/Weights/...
