...
namespace DeepUnity
{
    namespace CosyVoiceModeling
    {
        ...
        public class CosyVoiceLM : IDisposable
        {
            ...
            /// <summary>A6-max Phase 6 fused decode path (default ON): 6 dispatches/layer + 1
            /// head (145/token, was 411) with fused RMSNorms/residuals and coalesced staged-tile
            /// GEMVs. BIT-EXACT by construction (serial-order rms, per-output sequential-k dots,
            /// `precise` residual epilogues) — same logits, same tokens. false = the Phase-4
            /// per-op kernels, for bisection. Prefill always uses the original kernels.</summary>
            ...
            // gate*up (fused post-norm), down+residual. Bit-exact vs DispatchLayer(li, 1).
            void DispatchLayerFast(int li)
            {
                // 1. fused input RMSNorm + Q/K/V (+ biases): 4 Q groups + 1 K + 1 V
                ...
                cs.Dispatch(kDecOProj, (H + 255) / 256, 1, 1);
                ...
                cs.Dispatch(kDecGateUp, (MLP_DIM + 255) / 256, 1, 1);
                ...
                cs.Dispatch(kDecDown, (H + 255) / 256, 1, 1);
            }
            ...
            void DispatchHeadFast()   // fused final RMSNorm + speech head
            {
                ...
                cs.Dispatch(kDecHead, (VOCAB + 255) / 256, 1, 1);
            }
