...
namespace DeepUnity.ReinforcementLearning
{
    ...
    // DEPRECATED CPU PPO — kept for backward compatibility only, locked in the inspector.
    // The standard PPO is the full-GPU PPOTrainer (FullGPU/PPOTrainer.cs).
    internal sealed class PPOTrainerDepr : DeepUnityTrainer, IOnPolicy
    {
        // Keep the pre-rename checkpoint key so existing OptimStates files (shared with the
        // GPU PPOTrainer — same AdamW JSON format) still load.
...
#if UNITY_EDITOR
    [CustomEditor(typeof(PPOTrainerDepr), true), CanEditMultipleObjects]
