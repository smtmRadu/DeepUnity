...
namespace DeepUnity.ReinforcementLearning
{
    ...
    // DEPRECATED CPU SAC — kept for backward compatibility only, locked in the inspector.
    // The standard SAC is the full-GPU SACTrainer (FullGPU/SACTrainer.cs).
    internal sealed class SACTrainerDepr : DeepUnityTrainer, IOffPolicy
    {
        // Keep the pre-rename checkpoint key so existing OptimStates files (shared with the
        // GPU SACTrainer — same AdamW JSON format) still load.
        ...
        private void UpdatePolicy(Tensor states, long currentSacStep)
        {
            ...
            if (currentSacStep % 50 == 0)
            {
                ...
                AgentDebugLog("SACTrainerDepr.cs:UpdatePolicy", "sigmaClipMask", "{\"sigmaClipMaskMean\":" + sigmaClipMaskMean + ",\"step\":" + currentSacStep + "}", "D");
                AgentDebugLog("SACTrainerDepr.cs:UpdatePolicy", "LR", "{\"actorLR\":" + actorLR + ",\"sacStep\":" + currentSacStep + ",\"maxSteps\":" + maxSteps + ",\"stepRatio\":" + stepRatio + "}", "E");
                AgentDebugLog("SACTrainerDepr.cs:UpdatePolicy", "gradient magnitude", "{\"dQduL2\":" + dQduL2 + ",\"dJdMuL2\":" + dJdMuL2 + ",\"step\":" + currentSacStep + "}", "F");
                AgentDebugLog("SACTrainerDepr.cs:UpdatePolicy", "J trend", "{\"J\":" + jVal + ",\"minQ\":" + minQVal + ",\"entropyTerm\":" + entVal + ",\"step\":" + currentSacStep + "}", "B");
...
#if UNITY_EDITOR
    [CustomEditor(typeof(SACTrainerDepr), true), CanEditMultipleObjects]
    sealed class CustomSACTrainerDeprEditor : Editor
    {
