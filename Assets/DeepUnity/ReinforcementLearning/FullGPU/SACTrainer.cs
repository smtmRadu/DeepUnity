// SACTrainer — sibling to SACTrainerDepr, fully GPU-resident (FullGPU path).
//
// Built on the same proven stack as PPOTrainer (GPUMLP + GPUAdamW + RLBoosterCS)
...
//
// Deliberate config differences from the (broken-by-default) CPU SACTrainerDepr history:
...
// SACTrainerDepr so CPU vs GPU runs are directly comparable.
//
...
namespace DeepUnity.ReinforcementLearning
{
    internal sealed class SACTrainer : DeepUnityTrainer, IOffPolicy
    {
        ...
        protected override void Initialize(string[] optimizer_states)
        {
            ...
            // Target networks: fresh CPU clones -> GPU (recreated every session, like SACTrainerDepr).
            ...
            // Optimizer states from disk — shared JSON format & file key with the CPU SACTrainerDepr,
        ...
        {
            // Same gating / replay semantics as SACTrainerDepr for 1:1 comparability.
        ...
        // -------------------- save / load --------------------
        // Shares the optimizer-state files with the CPU SACTrainerDepr: order [q1q2, mu, sigma].
        ...
        string SerializeGpuAdam(Parameter[] pars, GPUParam[] gps, GPUAdamW gopt, float lr)
        {
            ...
            adam.description = "(AdamW) saved by SACTrainer (CPU-compatible)";
